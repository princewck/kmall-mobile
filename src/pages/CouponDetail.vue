    ...
    <div class="detail-content">
      ...
      <p class="sub-desc">
        <!-- <span class="origin-price">官方价: ¥
            <span :class="{del: data.coupon_price > 0}" v-text="prehandlePrice(img.price)"></span>
          </span> -->
        <span class="sold">月销量:
          <span v-text="data.volume"> 件 </span>
        </span>
      </p>
      <div class="coupon" v-if="data.coupon_info">
        <!--先这么写，后面研究下scheme跳转app的兼容性-->
        <a :href="data.coupon_click_url" target="_blank">
          <div class="coupon-desc">
            <em>
              <span></span>{{ getCouponPrice(data.coupon_info) }}</em>
            <span>使用期限</span>
            <span>{{ prep(data.coupon_start_time) }}-{{ prep(data.coupon_end_time) }}</span>
            <span>{{ data.coupon_info }}</span>
          </div>
      ...
      <div class="buy-button">
        ...
        <a :href="data.coupon_click_url" target="_new">去天猫看看</a>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      data: { pict_url: loading }
    }
  },
  mounted: function () {
    var pid = this.$route.params.id;
    let kw = decodeURIComponent(this.$route.params.kw);
    this.load(kw, pid);
  ...
  methods: {
    load(kw, id) {
      let vm = this;
      fetch(`/api/web/nice_coupons/${kw}/1`)
        .then(res => res.json())
        .then(({ results }) => {
          var rt = results.tbk_coupon.find((item) => (item.num_iid == id));
          vm.$set(vm, 'data', rt);
          if (!rt) {
            alert('该商品暂时不可用');
            vm.$router.go(-1);
